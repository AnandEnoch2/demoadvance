      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        <div className={`text-center mb-8 sm:mb-12 lg:mb-16 transform transition-all duration-1000 ${
          ...
          <p className="text-base sm:text-lg lg:text-xl text-gray-600 max-w-3xl mx-auto leading-relaxed px-4 sm:px-0">
            ...
            <span className="text-black font-semibold"> cutting-edge technology</span> and 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 lg:gap-12">
          {services.map((service, index) => (
            ...
            >
              <div className="relative bg-white rounded-2xl sm:rounded-3xl shadow-lg overflow-hidden transform group-hover:scale-105 transition-all duration-500 border border-gray-200 group-hover:border-black mx-4 sm:mx-0">
                {/* Animated Border */}

                <div className="relative z-10">
                  {/* Header with Image */}
                  <div className="relative h-48 sm:h-56 lg:h-64 bg-black overflow-hidden">
                    <img 
                      src={service.image}
                      alt={service.title}
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className={`w-16 h-16 sm:w-20 sm:h-20 lg:w-24 lg:h-24 bg-white rounded-2xl sm:rounded-3xl flex items-center justify-center transform group-hover:rotate-12 group-hover:scale-110 transition-all duration-500 shadow-2xl ${
                        hoveredCard === index ? 'animate-bounce' : ''
                      }`}>
                        <service.icon className="w-8 h-8 sm:w-10 sm:h-10 lg:w-12 lg:h-12 text-black" />
                      </div>
                    </div>
                    ...
                    <div className="absolute top-2 right-2 sm:top-4 sm:right-4 flex space-x-1 sm:space-x-2">
                      {Object.entries(service.stats).map(([key, value]) => (
                        <div key={key} className="bg-white border border-black rounded-lg px-2 sm:px-3 py-1">
                          <div className="text-black text-xs sm:text-sm font-bold">{value}</div>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="p-4 sm:p-6 lg:p-8">
                    <h3 className="text-xl sm:text-2xl lg:text-3xl font-bold text-black mb-3 sm:mb-4 transition-all duration-300">
                      {service.title}
                    </h3>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 mb-6 sm:mb-8">
                      {service.features.map((feature, idx) => (
                        <div 
                          key={idx}
                          className="flex items-center space-x-2 sm:space-x-3 p-2 sm:p-3 rounded-lg sm:rounded-xl bg-gray-100 group-hover:bg-white group-hover:shadow-lg transition-all duration-300 transform group-hover:scale-105"
                    ...
                    <button 
                      ...
                      className="w-full bg-black text-white py-3 sm:py-4 rounded-xl sm:rounded-2xl font-bold text-sm sm:text-base lg:text-lg hover:bg-gray-800 transition-all duration-300 transform group-hover:scale-105 flex items-center justify-center space-x-2 relative overflow-hidden"
                    >
        ...
        <div className={`text-center mt-8 sm:mt-12 lg:mt-16 transform transition-all duration-1000 delay-500 mx-4 sm:mx-0 ${
          ...
          <div className="bg-white border-2 border-black rounded-2xl sm:rounded-3xl p-6 sm:p-8 text-black shadow-lg">
            <h3 className="text-2xl sm:text-3xl font-bold mb-3 sm:mb-4">Ready to Get Started?</h3>
            <p className="text-base sm:text-lg lg:text-xl mb-4 sm:mb-6 text-gray-700">Join thousands of satisfied customers who trust us with their financial needs</p>
            <button 
              onClick={() => scrollToSection('contact')}
              className="bg-black text-white px-6 sm:px-8 py-3 sm:py-4 rounded-xl sm:rounded-2xl font-bold text-sm sm:text-base lg:text-lg hover:bg-gray-800 transition-all duration-300 transform hover:scale-105"
            >
